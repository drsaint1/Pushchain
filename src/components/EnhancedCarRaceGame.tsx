...
const EnhancedCarRaceGame: React.FC<EnhancedCarRaceGameProps> = ({
  ...
  useEffect(() => {
    if (isTxConfirmed && currentTxHash) {
      ...
      if (autoSubmitResults) {
        showPopup("✅ Results auto-saved on Pushchain!");
      } else {
    ...
    try {
      ...
      if (error.message) {
        if (
          ...
          errorMessage =
            "💰 Insufficient funds for breeding fee (need 0.01 PC)";
        } else if (
  ...
  const mintCarWithConfirmation = async (
    ...
    try {
      ...
      if (
        ...
        errorMessage = `❌ Insufficient funds to purchase ${carName}. You need 0.01 PC.`;
      } else {
  ...
  const submitRaceResult = useCallback(async () => {
    ...
    try {
      ...
      if (autoSubmitResults) {
        showPopup("⏳ Auto-saving to Pushchain blockchain...");
      } else {
        showPopup("⏳ Submitting to Pushchain blockchain...");
      }
    } catch (error) {
      setSubmissionStatus("error");
      setIsSubmittingScore(false);
                  ...
                  {autoSubmitResults ? (
                    ...
                    >
                      {submissionStatus === "waiting_wallet"
                        ...
                        ? "⏳ Auto-saving to PushChain..."
                        : submissionStatus === "success"
                        ? "✅ Results auto-saved on PushChain!"
