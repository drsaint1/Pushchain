...
contract PushchainRacing is ERC721URIStorage, Ownable, ReentrancyGuard, Pausable {
    struct RaceCar {
        uint256 speed;
        uint256 handling;
    ...
    constructor() ERC721("PushGame Racing Cars", "PGC") Ownable(msg.sender) {
    ...
    function _generateTokenURI(RaceCar memory car, uint256 tokenId) internal pure returns (string memory) {
        ...
        return string(abi.encodePacked("https://api.pushchainracing.com/metadata/", _toString(tokenId)));
    }
